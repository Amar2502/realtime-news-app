...
import { Clock, TrendingUp, ChevronRight, Briefcase, Star, ArrowLeft } from "lucide-react";
...
const BusinessNewsFeed = () => {
  ...
  useEffect(() => {
    ...
    socket.on("new-news", (newItem) => {
      console.log("🔔 Received new-news:", newItem);
            ...
            <div className="flex justify-between items-start">
              <div className="flex-1">
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <div className="px-3 py-1 bg-indigo-100 text-indigo-700 text-sm font-medium rounded-full flex items-center gap-1">
                    <TrendingUp className="w-3 h-3" />
                    Live Updates
                  </div>
                </div>
                <h1 className="text-5xl font-bold bg-gradient-to-r from-gray-900 via-indigo-800 to-blue-800 bg-clip-text text-transparent mb-3">
                  Business News
                </h1>
                <p className="text-xl text-gray-600 max-w-2xl leading-relaxed">
                  Stay ahead with the latest business insights, market trends, and financial news from around the world.
                </p>
              </div>
              {!isLoading && (
          ...
          <div className="flex justify-center my-16">
            <div className="flex flex-col items-center space-y-4">
              ...
              <div className="text-center">
                <p className="text-lg font-medium text-gray-700">Loading business news...</p>
                <p className="text-sm text-gray-500">Getting the latest market updates</p>
              </div>
            </div>
          </div>
        ) : (
            ...
            {news.length === 0 ? (
              <div className="flex justify-center my-16">
                <div className="text-center p-12 bg-white/60 backdrop-blur-sm rounded-3xl border border-white/20 shadow-lg max-w-md">
                  ...
                  <h3 className="text-xl font-semibold text-gray-700 mb-2">No Business News Available</h3>
                  <p className="text-gray-500">Check back soon for the latest business updates</p>
                </div>
              </div>
            ) : (
              /* News Grid */
                  ...
                  >
                    ...
                    <div className="relative p-6 flex flex-col flex-grow">
                      <div className="flex items-center justify-between mb-4">
                        <span className="px-3 py-1.5 text-xs font-bold rounded-full shadow-sm bg-gradient-to-r from-indigo-100 to-indigo-200 text-indigo-800">
                          {item.category}
                        </span>
                      ...
                      <div className="flex justify-between items-center pt-4 border-t border-gray-100">
                        ...
                        <Link
                          to={`/news/${item._id}`}
                          className="inline-flex items-center text-sm font-semibold text-indigo-600 hover:text-indigo-700 group-hover:underline transition-colors duration-300"
                        >
                          Read more
