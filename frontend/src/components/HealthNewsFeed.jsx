...
import { Clock, TrendingUp, ChevronRight, Heart, Star, ArrowLeft, Sparkles, Activity } from "lucide-react";
          ...
          <div className="relative p-8 rounded-3xl backdrop-blur-sm bg-white/40 border border-white/20 shadow-lg">
            <div className="flex justify-between items-start">
              <div className="flex-1">
                ...
                <h1 className="text-5xl font-bold bg-gradient-to-r from-gray-900 via-green-800 to-emerald-800 bg-clip-text text-transparent mb-3">
                  Health News
                </h1>
                <p className="text-xl text-gray-600 max-w-2xl leading-relaxed">
                  Stay informed with the latest medical breakthroughs, wellness tips, and health research from trusted sources worldwide.
                </p>
              </div>
              {!isLoading && (
          ...
          <div className="flex justify-center my-16">
            <div className="flex flex-col items-center space-y-4">
              ...
              <div className="text-center">
                <p className="text-lg font-medium text-gray-700">Loading health news...</p>
                <p className="text-sm text-gray-500">Getting the latest wellness updates</p>
              </div>
            </div>
          </div>
        ) : (
            ...
            {news.length === 0 ? (
              <div className="flex justify-center my-16">
                <div className="text-center p-12 bg-white/60 backdrop-blur-sm rounded-3xl border border-white/20 shadow-lg max-w-md">
                  ...
                  <h3 className="text-xl font-semibold text-gray-700 mb-2">No Health News Available</h3>
                  <p className="text-gray-500">Check back soon for the latest medical updates and wellness insights</p>
                </div>
              </div>
            ) : (
              /* News Grid */
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {news.map((item, index) => (
                  ...
                  >
                    {/* Gradient overlay */}
                    <div className="absolute inset-0 bg-gradient-to-br from-transparent via-transparent to-green-50/30 opacity-0 group-hover:opacity-100 transition-opacity duration-500"></div>
                    
                    <div className="relative p-6 flex flex-col flex-grow">
                      <div className="flex items-center justify-between mb-4">
                        <span className="px-3 py-1.5 text-xs font-bold rounded-full shadow-sm bg-gradient-to-r from-green-100 to-green-200 text-green-800">
                          {item.category}
                        </span>
                      ...
                      <h3 className="text-xl font-bold text-gray-900 mb-3 leading-tight group-hover:text-green-700 transition-colors duration-300">
                        {item.title}
                      </h3>
                      ...
                      <div className="flex justify-between items-center pt-4 border-t border-gray-100">
                        ...
                        <Link
                          to={`/news/${item._id}`}
                          className="inline-flex items-center text-sm font-semibold text-green-600 hover:text-green-700 group-hover:underline transition-colors duration-300"
                        >
                          Read more
